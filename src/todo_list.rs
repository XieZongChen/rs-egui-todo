...
pub struct TodoList {
    // 定义一个公开的结构体 TodoList
    items: Vec<String>, // 定义一个字段 items，类型为 Vec<String>，用于存储待办事项列表
...
impl Default for TodoList {
    fn default() -> Self {
        Self {
            items: vec!["Item 1".to_string(), "Item 2".to_string()], // 初始化 items 字段，包含两个示例待办事项
            ...
            ui.horizontal(|ui| {
                ...
                if ui.button("Add").clicked() {
                    ...
                    if !self.new_item.is_empty() {
                        // 如果 new_item 不为空
                        self.items.push(self.new_item.clone()); // 将 new_item 复制并添加到 items 列表中
